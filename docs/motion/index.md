...
const chain = TypeScriptChain(llm, schema, "SentimentResponse", true);
const res = await chain.call("let's play?!");
if (!res.success) {
...
```plaintext
 system (system_schema): You need to process user requests and then translates result into JSON objects of type "SentimentResponse" according to the following TypeScript definitions:
"""
